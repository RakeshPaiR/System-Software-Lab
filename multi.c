...
void main()
{
	...
	int  k,size,s[10],s1[10][10],ch1,ch,flag=1;
	char dir[10],dir1[10][10],dir2[10][10][10],dir3[10][10][10][10],sea[10];
	int i,j;
	 ...
	 {	//printf("\nDirectory %d : %s",i,dir1[i]);
	  ...
	  {	printf("\nDirectory %d : %s",i,dir1[i]);
		...
		{	//printf("\nDirectory %d : %s",i,dir1[i]);
			...
			{        
			//printf("\nFile name %d : %s",j,dir2[i][j]);
				if(strcmp(sea,dir2[i][j])==0)
				{	s[i]--;
					for(k=j;k<s[i];k++)

					{
						strcpy(dir2[i][k+1],dir2[i][k]);
					}
					break;
				}
			}
